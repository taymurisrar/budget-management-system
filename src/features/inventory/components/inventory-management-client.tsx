...
type InventoryCategoryOption = { id: string; name: string; iconKey: string | null };
...
const nextItems = items.filter((item) => {
  ...
  const addSuggestedTag = (tag: string) =>
    setTagsText((current) => {
      const tags = current.split(",").map((value) => value.trim()).filter(Boolean);
      return tags.includes(tag) ? current : tags.length > 0 ? `${tags.join(", ")}, ${tag}` : tag;
    });
...
<td className="px-6 py-4"><div className="flex justify-end gap-2">
    ...
    >
      ...
      {selectedItem ? <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/55 p-4 sm:items-center"><div role="dialog" aria-modal="true" className="max-h-[90vh] w-full max-w-4xl overflow-auto rounded-[30px] border border-white/15 bg-[rgba(255,255,255,0.96)] p-6 shadow-[0_28px_100px_rgba(15,23,42,0.35)] backdrop-blur dark:bg-slate-950">
        <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.22em] text-slate-500">Inventory record</p><h3 className="mt-2 text-2xl font-semibold">{selectedItem.name}</h3><p className="text-muted mt-2 text-sm">{selectedItem.categoryName}{selectedItem.subcategory ? ` | ${selectedItem.subcategory}` : ""}{selectedItem.brand ? ` | ${selectedItem.brand}` : ""}</p></div><button type="button" onClick={closeModal} className="rounded-2xl border border-[var(--border)] p-2.5"><X className="h-4 w-4" /></button></div>
        ...
        {!isEditing ? <>
          ...
          <div className="mt-6 grid gap-6 md:grid-cols-2"><div className="rounded-[28px] border border-[var(--border)] bg-white/65 p-5 text-sm dark:bg-white/5"><p className="font-semibold uppercase tracking-[0.18em] text-slate-500">Timeline</p><div className="mt-4 space-y-3"><p><span className="text-slate-500">Purchased:</span> {formatInventoryDate(selectedItem.lastPurchaseDate)}</p><p><span className="text-slate-500">Finished / ended:</span> {formatInventoryDate(selectedItem.lastConsumptionDate)}</p><p><span className="text-slate-500">Restock date:</span> {formatInventoryDate(selectedItem.nextRestockDate)}</p><p><span className="text-slate-500">Expiry:</span> {formatInventoryDate(selectedItem.expiryDate)}</p></div></div><div className="rounded-[28px] border border-[var(--border)] bg-white/65 p-5 text-sm dark:bg-white/5"><p className="font-semibold uppercase tracking-[0.18em] text-slate-500">Notes</p><p className="text-muted mt-4 whitespace-pre-wrap">{selectedItem.notes || "No notes saved for this item."}</p></div></div>
          <div className="mt-6 rounded-[28px] border border-[var(--border)] bg-white/65 p-5 text-sm dark:bg-white/5"><p className="font-semibold uppercase tracking-[0.18em] text-slate-500">Configuration history</p><div className="mt-4 space-y-3">{selectedItem.revisions.length > 0 ? selectedItem.revisions.map((revision) => <div key={revision.id} className="rounded-2xl border border-[var(--border)] bg-white/70 px-4 py-3 dark:bg-white/5"><div className="flex items-start justify-between gap-3"><div className="min-w-0 flex-1">{editingRevisionId === revision.id ? <><input value={revisionSummaryDraft} onChange={(e) => setRevisionSummaryDraft(e.target.value)} className="w-full rounded-xl border border-[var(--border)] bg-white px-3 py-2 text-sm dark:bg-white/5" /><div className="mt-2 flex gap-2"><button type="button" onClick={saveRevisionSummary} disabled={isSavingHistory} className="rounded-xl bg-black px-3 py-2 text-xs font-medium text-white disabled:opacity-60 dark:bg-white dark:text-black">{isSavingHistory ? "Saving..." : "Save"}</button><button type="button" onClick={() => { setEditingRevisionId(null); setRevisionSummaryDraft(""); }} className="rounded-xl border border-[var(--border)] px-3 py-2 text-xs">Cancel</button></div></> : <p className="font-medium">{revision.changeSummary || "Snapshot saved"}</p>}<p className="text-muted mt-1 text-xs">{formatInventoryDate(revision.createdAt)} | {revision.brand || "No brand"} | {formatNumber(revision.currentQuantity)} {formatInventoryUnit(revision.unit)} | Users: {revision.householdUserCount}</p></div><div className="flex flex-col items-end gap-2"><span className={`rounded-full px-2.5 py-1 text-xs font-medium ${revision.revisionType === "restocked" ? "bg-blue-50 text-blue-700 dark:bg-blue-500/10 dark:text-blue-300" : revision.revisionType === "created" ? "bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300" : "bg-slate-100 text-slate-700 dark:bg-white/10 dark:text-slate-300"}`}>{revision.revisionType}</span><div className="flex gap-2"><button type="button" onClick={() => startRevisionEdit(revision)} className="rounded-xl border border-[var(--border)] px-2.5 py-1.5 text-xs">Edit</button><button type="button" onClick={() => deleteRevision(revision.id)} disabled={isSavingHistory} className="rounded-xl border border-red-200 px-2.5 py-1.5 text-xs text-red-700 disabled:opacity-50 dark:border-red-500/30 dark:text-red-300">Remove</button></div></div></div></div>) : <p className="text-muted">No history yet.</p>}</div></div>
          <div className="mt-6 flex flex-wrap justify-end gap-3"><button type="button" onClick={deleteItem} disabled={isDeleting} className="rounded-2xl border border-red-200 px-4 py-2.5 text-sm font-medium text-red-700 disabled:opacity-50 dark:border-red-500/30 dark:text-red-300"><span className="inline-flex items-center gap-2"><Trash2 className="h-4 w-4" />{isDeleting ? "Deleting..." : "Delete"}</span></button><button type="button" onClick={() => openRestock(selectedItem)} className="rounded-2xl border border-[var(--border)] px-4 py-2.5 text-sm font-medium"><span className="inline-flex items-center gap-2"><PackagePlus className="h-4 w-4" />Restock</span></button><button type="button" onClick={() => setIsEditing(true)} className="rounded-2xl bg-black px-4 py-2.5 text-sm font-medium text-white dark:bg-white dark:text-black">Edit record</button></div>
        </> : isRestocking ? <>
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            ...
            <div><InventoryFieldHelp label="Purchase date" help="When this restock happened." /><input type="date" value={restockFormData?.purchaseDate ?? getTodayDateInputValue()} onChange={(e) => updateRestockValue("purchaseDate", e.target.value)} className="w-full rounded-2xl border border-[var(--border)] bg-white/70 px-4 py-3 dark:bg-white/5" max={getTodayDateInputValue()} />{fieldErrors.purchaseDate ? <p className="mt-2 text-sm text-red-600">{fieldErrors.purchaseDate[0]}</p> : null}</div>
            ...
            <div className="md:col-span-2"><button type="button" onClick={() => setShowAdvancedRestock((current) => !current)} className="rounded-2xl border border-[var(--border)] px-4 py-2.5 text-sm font-medium">{showAdvancedRestock ? "Hide advanced restock fields" : "Show advanced restock fields"}</button></div>
            {showAdvancedRestock ? <>
              ...
              <div><InventoryFieldHelp label="Expiry date" help="Optional expiry date for this newly purchased batch." optional /><input type="date" value={restockFormData?.expiryDate ?? ""} onChange={(e) => updateRestockValue("expiryDate", e.target.value || undefined)} className="w-full rounded-2xl border border-[var(--border)] bg-white/70 px-4 py-3 dark:bg-white/5" min={restockFormData?.purchaseDate || undefined} />{fieldErrors.expiryDate ? <p className="mt-2 text-sm text-red-600">{fieldErrors.expiryDate[0]}</p> : null}</div>
            </> : null}
          </div>
          <div className="mt-6 flex flex-wrap justify-end gap-3"><button type="button" onClick={() => setIsRestocking(false)} className="rounded-2xl border border-[var(--border)] px-4 py-2.5 text-sm font-medium">Cancel</button><button type="button" onClick={saveRestock} disabled={isSavingRestock} className="rounded-2xl bg-black px-4 py-2.5 text-sm font-medium text-white disabled:opacity-60 dark:bg-white dark:text-black">{isSavingRestock ? "Saving..." : "Save restock"}</button></div>
        </> : <>
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            ...
            <div><InventoryFieldHelp label="Subcategory" help="Add a practical grouping like oral care, laundry, or guest washroom." optional /><input value={formData.subcategory ?? ""} onChange={(e) => updateFormValue("subcategory", e.target.value)} className="w-full rounded-2xl border border-[var(--border)] bg-white/70 px-4 py-3 dark:bg-white/5" />{categoryProfile ? <div className="mt-2 flex flex-wrap gap-2">{categoryProfile.subcategories.map((subcategory) => <button key={subcategory} type="button" onClick={() => updateFormValue("subcategory", subcategory)} className="rounded-full border border-[var(--border)] px-3 py-1 text-xs">{subcategory}</button>)}</div> : null}</div>
            <div><InventoryFieldHelp label="Tags" help="Short labels like bathroom, monthly refill, or guest stock improve search and filtering." optional /><input value={tagsText} onChange={(e) => setTagsText(e.target.value)} className="w-full rounded-2xl border border-[var(--border)] bg-white/70 px-4 py-3 dark:bg-white/5" />{categoryProfile ? <div className="mt-2 flex flex-wrap gap-2">{categoryProfile.tags.map((tag) => <button key={tag} type="button" onClick={() => addSuggestedTag(tag)} className="rounded-full bg-blue-50 px-3 py-1 text-xs text-blue-700 dark:bg-blue-500/10 dark:text-blue-300">{tag}</button>)}</div> : null}</div>
            ...
            <div><InventoryFieldHelp label="Purchase date" help="When this pack or item was bought. Used to build consumption history." optional /><input type="date" value={toDateInputValue(formData.lastPurchaseDate)} onChange={(e) => updateFormValue("lastPurchaseDate", e.target.value || undefined)} className="w-full rounded-2xl border border-[var(--border)] bg-white/70 px-4 py-3 dark:bg-white/5" max={getTodayDateInputValue()} />{fieldErrors.lastPurchaseDate ? <p className="mt-2 text-sm text-red-600">{fieldErrors.lastPurchaseDate[0]}</p> : null}</div>
            <div><InventoryFieldHelp label="Finished / ended date" help="Set this when a pack was fully used up. If unit size is known, the app can suggest daily usage from purchase to finish." optional /><input type="date" value={toDateInputValue(formData.lastConsumptionDate)} onChange={(e) => updateFormValue("lastConsumptionDate", e.target.value || undefined)} className="w-full rounded-2xl border border-[var(--border)] bg-white/70 px-4 py-3 dark:bg-white/5" min={toDateInputValue(formData.lastPurchaseDate) || undefined} max={getTodayDateInputValue()} />{fieldErrors.lastConsumptionDate ? <p className="mt-2 text-sm text-red-600">{fieldErrors.lastConsumptionDate[0]}</p> : null}</div>
            <div><InventoryFieldHelp label="Next restock date" help="Auto-calculated from the item cadence or from stock depletion and daily use. You can still adjust it manually if needed." optional /><input type="date" value={toDateInputValue(formData.nextRestockDate)} onChange={(e) => updateFormValue("nextRestockDate", e.target.value || undefined)} className="w-full rounded-2xl border border-[var(--border)] bg-white/70 px-4 py-3 dark:bg-white/5" min={getTodayDateInputValue()} />{fieldErrors.nextRestockDate ? <p className="mt-2 text-sm text-red-600">{fieldErrors.nextRestockDate[0]}</p> : null}<p className="text-muted mt-2 text-xs">Live estimate: {calculatedNextRestockDate ? formatInventoryDate(calculatedNextRestockDate) : "Need a cadence or daily use to calculate."}</p></div>
            <div><InventoryFieldHelp label="Expiry date" help="Track shelf-life sensitive items like medicine, food, and personal care products." optional /><input type="date" value={toDateInputValue(formData.expiryDate)} onChange={(e) => updateFormValue("expiryDate", e.target.value || undefined)} className="w-full rounded-2xl border border-[var(--border)] bg-white/70 px-4 py-3 dark:bg-white/5" min={toDateInputValue(formData.lastPurchaseDate) || undefined} />{fieldErrors.expiryDate ? <p className="mt-2 text-sm text-red-600">{fieldErrors.expiryDate[0]}</p> : null}</div>
          ...
          <div className="mt-6 flex flex-wrap justify-between gap-3"><button type="button" onClick={deleteItem} disabled={isDeleting} className="rounded-2xl border border-red-200 px-4 py-2.5 text-sm font-medium text-red-700 disabled:opacity-50 dark:border-red-500/30 dark:text-red-300">{isDeleting ? "Deleting..." : "Delete"}</button><div className="flex gap-3"><button type="button" onClick={() => { setFormData(itemToForm(selectedItem)); setTagsText(Array.isArray(selectedItem.tags) ? selectedItem.tags.join(", ") : ""); setFieldErrors({}); setServerError(""); setIsEditing(false); }} className="rounded-2xl border border-[var(--border)] px-4 py-2.5 text-sm font-medium">Cancel</button><button type="button" onClick={saveChanges} className="rounded-2xl bg-black px-4 py-2.5 text-sm font-medium text-white dark:bg-white dark:text-black">Save changes</button></div></div>
        </>}
      </div></div> : null}
    </div>
  );
